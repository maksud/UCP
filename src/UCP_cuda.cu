...
__global__
void gpu_sum_scan_blelloch(SumDataType* const d_out,
	const SumDataType* const d_in,
	SumDataType* const d_block_sums,
...
__global__
void gpu_add_block_sums(SumDataType* const d_out,
	const SumDataType* const d_in,
	SumDataType* const d_block_sums,
	const size_t numElems)
{
...
__global__
void gpu_prescan(SumDataType* const d_out,
	const SumDataType* const d_in,
	SumDataType* const d_block_sums,
	const unsigned long len,
	const unsigned long shmem_sz,
	const unsigned long max_elems_per_block)
{
...
void sum_scan_blelloch(SumDataType*  d_out,
	SumDataType*  d_in,
	const size_t numElems)
{
	// Zero out d_out
...
__global__ void checkUCP(SumDataType* d_out, SumDataType Capacity, int N, int P, BoundaryMessage* startBoundary, BoundaryMessage* endBoundary){
    ...
    for(int m=idx+1; m<N; m+=BLOCK_GRID){
        int pMm1 = (d_out[m-1]-0.0001) / Capacity;
        int pM = (d_out[m]-0.0001) / Capacity;
...
int main (){
    ...
    CUDA_CHECK(cudaMalloc(&d_num, (N+1) * sizeof(SumDataType)));
    CUDA_CHECK(cudaMalloc(&d_res, (N+1) * sizeof(SumDataType)));

    CUDA_CHECK(cudaMemcpy(d_num, num, (N+1)*sizeof(SumDataType), cudaMemcpyHostToDevice));

    // 2. Record the start event
    cudaEventRecord(start);

    calcCost<<<5120, 512>>>(d_num, N);

    sum_scan_blelloch(d_res, d_num, (N+1));
    ...
    CUDA_CHECK(cudaMemcpy(res, d_res, (N+1)*sizeof(SumDataType), cudaMemcpyDeviceToHost));
    ...
    cudaEventRecord(start);
    checkUCP<<<512, 512>>>(d_res, Capacity, N+1, P, d_boundaries, d_boundaries+P);
    cudaDeviceSynchronize();
    // 4. Record the stop event
    cudaEventRecord(stop);
    // 5. Wait for the event to complete
    ...
    if (rank >= 0)
    {
        ...
        if (x >= 0 && x< N)
        {
            ...
            double eStart = startBoundary[rank].from;
            double eEnd = startBoundary[rank].to;

            //totalWorkLoad += eEnd - eStart;

            //std::cout << "#1. Rank " << rank << " Task " << x << " SubTask: eStart " << eStart << " eEnd " << eEnd
            //          << std::endl;
        }
    }
    ...
    if (rank <= P - 1)
    {
        ...
        if (x >= 0 && x < N)
        {
            ...
            double eStart = endBoundary[rank].from;
            double eEnd = endBoundary[rank].to;

            //totalWorkLoad += eEnd - eStart;

            //std::cout << "#3. Rank " << rank << " Task " << x << " SubTask: eStart " << eStart << " eEnd " << eEnd
            //          << std::endl;
        }
    }
