...
{
    // Zero out d_out
    CUDA_CHECK(hipMemset(d_out, 0, numElems * sizeof(SumDataType)));
    ...
    CUDA_CHECK(hipMalloc(&d_block_sums, sizeof(SumDataType) * grid_sz));
    CUDA_CHECK(hipMemset(d_block_sums, 0, sizeof(SumDataType) * grid_sz));
    ...
    if (grid_sz <= max_elems_per_block)
    {
        ...
        CUDA_CHECK(hipMalloc(&d_dummy_blocks_sums, sizeof(SumDataType)));
        CUDA_CHECK(hipMemset(d_dummy_blocks_sums, 0, sizeof(SumDataType)));
        ...
        CUDA_CHECK(hipFree(d_dummy_blocks_sums));
    }
    ...
    else
    {
        ...
        CUDA_CHECK(hipMalloc(&d_in_block_sums, sizeof(SumDataType) * grid_sz));
        CUDA_CHECK(hipMemcpy(d_in_block_sums, d_block_sums, sizeof(SumDataType) * grid_sz, hipMemcpyDeviceToDevice));
        sum_scan_blelloch(d_block_sums, d_in_block_sums, grid_sz);
        CUDA_CHECK(hipFree(d_in_block_sums));
    }
...
int main(int argc, char *argv[])
{
    ...
    CUDA_CHECK(hipMalloc(&d_input, (N + 1) * sizeof(SumDataType)));
    CUDA_CHECK(hipMalloc(&d_output, (N + 1) * sizeof(SumDataType)));

    CUDA_CHECK(hipMemcpy(d_input, num, (N + 1) * sizeof(SumDataType), hipMemcpyHostToDevice));
    ...
    // hipMalloc(&d_input, num_items * sizeof(SumDataType));
    // hipMalloc(&d_output, num_items * sizeof(SumDataType));

#if 1
    // Determine temp storage size for CUB inclusive scan
    void *d_temp_storage = nullptr;
    size_t temp_storage_bytes = 0;
    // hipcub::DeviceScan::ExclusiveSum(nullptr, temp_storage_bytes, d_input, d_output, N + 1);
    hipcub::DeviceScan::InclusiveSum(nullptr, temp_storage_bytes, d_input, d_output, N + 1);
    printf("Temp storage size: %zu bytes\n", temp_storage_bytes);
    hipMalloc(&d_temp_storage, temp_storage_bytes);
    // hipcub::DeviceScan::ExclusiveSum(d_temp_storage, temp_storage_bytes, d_input, d_output, N + 1);
    hipcub::DeviceScan::InclusiveSum(d_temp_storage, temp_storage_bytes, d_input, d_output, N + 1);
#else
    ...
    CUDA_CHECK(hipMalloc(&d_boundaries, (2 * P) * sizeof(BoundaryMessage)));
    CUDA_CHECK(hipMemcpy(d_boundaries, boundaries, (2 * P) * sizeof(BoundaryMessage), hipMemcpyHostToDevice));
